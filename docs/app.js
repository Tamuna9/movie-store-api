...
function saveCart() {
    window.localStorage.setItem("moviestar-cart", JSON.stringify(state.cart.map((movie) => movie.id)));
}
...
function addToCart(movieId) {
    ...
    if (!movie || !movie.availability || state.cart.some((item) => item.id === movieId)) return;
...
async function completeCheckout() {
    ...
    renderCart();
    closeCart();
    showToast("Demo purchase completed successfully");
...
async function initialize() {
    try {
        state.movies = await requestJson("./movies.json");
        let savedIds = [];
        try {
            savedIds = JSON.parse(window.localStorage.getItem("moviestar-cart") || "[]");
        } catch {
            window.localStorage.removeItem("moviestar-cart");
        }
        state.cart = savedIds
            .map((id) => state.movies.find((movie) => movie.id === Number(id)))
            .filter((movie) => movie && movie.availability);
